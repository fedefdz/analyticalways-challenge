...
namespace AcmeSchool.UnitTests.Application.UseCases
{
    public class PayRegistrationFeeCourseUseCaseTests
    {
        ...
        public async Task ExecuteAsync_WhenPaymentAmountIsLessThanCourseRegistrationFee_ThrowsPaymentAmountInsufficientException()
        {
            ...
            SetProperty(course, nameof(Course.RegistrationFee), 1000m);
        ...
        public async Task ExecuteAsync_WhenPaymentIsNotPending_ThrowsOperationNotAllowedException()
        {
            ...
            var command = _fixture.Create<PayRegistrationFeeCourseCommand>();
            Course course = CreateCourseWithId(command.CourseId);
            SetProperty(course, nameof(Course.RegistrationFee), command.RegistrationFeePaymentRequest.Amount);
            Student student = CreateStudentWithId(command.StudentId);
            RegistrationFeePayment registrationFeePayment = CreateRegistrationFeePaymentWithId(command.RegistrationFeePaymentRequest.PaymentId);
            SetProperty(registrationFeePayment, nameof(RegistrationFeePayment.Status), PaymentStatus.Approved);
        ...
        public async Task ExecuteAsync_WhenPaymentAmountMatchesCourseRegistrationFee_ThenProcessPayment()
        {
            ...
            Student student = CreateStudentWithId(command.StudentId);
            RegistrationFeePayment registrationFeePayment = CreateRegistrationFeePaymentWithId(command.RegistrationFeePaymentRequest.PaymentId);
            SetProperty(registrationFeePayment, nameof(RegistrationFeePayment.CourseId), course.Id);
            SetProperty(registrationFeePayment, nameof(RegistrationFeePayment.StudentId), student.Id);
            SetProperty(registrationFeePayment, nameof(RegistrationFeePayment.Status), PaymentStatus.Pending);
            SetProperty(registrationFeePayment, nameof(RegistrationFeePayment.Amount), command.RegistrationFeePaymentRequest.Amount);
        ...
        public async Task ExecuteAsync_WhenPaymentResultIsApproved_ThenConfirmRegistrationFeePAyment()
        {
            ...
            PaymentResult paymentResult = CreatePaymentResultWithId(command.RegistrationFeePaymentRequest.PaymentId);
            SetProperty(paymentResult, nameof(PaymentResult.Success), true);
            SetProperty(paymentResult, nameof(PaymentResult.ResultCode), PaymentResultCodes.Success);
        ...
        public async Task ExecuteAsync_WhenPaymentResultIsNotApproved_ThenShouldNotConfirmRegistrationFeePAyment()
        {
            ...
            {
                SetProperty(paymentResult, nameof(PaymentResult.Success), false);
                SetProperty(paymentResult, nameof(PaymentResult.ResultCode), PaymentResultCodes.InsufficientFunds);
            }
        ...
        private RegistrationFeePayment CreateRegistrationFeePaymentWithId(Guid paymentId)
        {
            ...
            SetProperty(reagistrationFeePayment, nameof(RegistrationFeePayment.PaymentId), paymentId);
